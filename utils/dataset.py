import torch
import cv2
import os
import glob
from torch.utils.data import Dataset
import random


class ISBI_Loader(Dataset):
    def __init__(self, data_path, mode='train'):
        self.mode = mode
        # 根据mode加载不同的数据
        if mode == 'train':
            self.data_path = os.path.join(data_path, 'train')
            self.imgs_path = glob.glob(os.path.join(data_path, 'Training_Images/*.jpg'))
        else:
            self.data_path = os.path.join(data_path, 'val')
            self.imgs_path = glob.glob(os.path.join(data_path, 'Val_Images/*.jpg'))

    def augment(self, image, flipCode):
        # 使用cv2.flip进行数据增强，filpCode为1水平翻转，0垂直翻转，-1水平+垂直翻转
        flip = cv2.flip(image, flipCode)
        return flip

    def __getitem__(self, index):
        image_path = self.imgs_path[index]
        # 修改标签路径构建逻辑
        if self.mode == 'train':
            label_path = image_path.replace('Training_Images', 'Training_Labels').replace('.jpg', '.png')
        else:
            label_path = image_path.replace('Val_Images', 'Val_Labels').replace('.jpg', '.png')
        label_path = label_path.replace('.jpg', '.png')  # 更新标签文件的逻辑

        # 读取训练图片和标签图片
        image = cv2.imread(image_path)
        label = cv2.imread(label_path)

        # 检查图像是否被正确读取
        if image is None:
            raise ValueError(f"无法读取图像: {image_path}")
        if label is None:
            raise ValueError(f"无法读取标签: {label_path}")

        image = cv2.resize(image, (512, 512))
        label = cv2.resize(label, (512, 512), interpolation=cv2.INTER_NEAREST)

        # 将数据转为单通道的图片
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        label = cv2.cvtColor(label, cv2.COLOR_BGR2GRAY)

        # 处理标签，将像素值为255的改为1
        if label.max() > 1:
            label = label / 255
        # 随机进行数据增强，为2时不做处理
        flipCode = random.choice([-1, 0, 1, 2])
        if flipCode != 2:
            image = self.augment(image, flipCode)
            label = self.augment(label, flipCode)
        image = image.reshape(1, image.shape[0], image.shape[1])
        label = label.reshape(1, label.shape[0], label.shape[1])
        return image, label

    def __len__(self):
        # 返回训练集大小
        return len(self.imgs_path)


if __name__ == "__main__":
    isbi_dataset = ISBI_Loader("data/train/")
    print("数据个数：", len(isbi_dataset))
    train_loader = torch.utils.data.DataLoader(dataset=isbi_dataset,
                                               batch_size=2,
                                               shuffle=True)
    for image, label in train_loader:
        print(image.shape)