...
class R_ECAUNet(nn.Module):  # 修改类名
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(R_ECAUNet, self).__init__()
        ...
        self.eca4 = ECABlock(2048)
        
        # 替换解码器部分的SE为ECA
        self.up_eca1 = ECABlock(1024)
        self.up_eca2 = ECABlock(512)
        self.up_eca3 = ECABlock(256)
        self.up_eca4 = ECABlock(64)

    def forward(self, x):
        # 编码器部分
        x0 = self.layer0(x)
        x1 = self.layer1(x0)
        x1 = self.eca1(x1)  # 使用ECA模块
        
        x2 = self.layer2(x1)
        x2 = self.eca2(x2)  # 使用ECA模块
        
        x3 = self.layer3(x2)
        x3 = self.eca3(x3)  # 使用ECA模块
        
        x4 = self.layer4(x3)
        x4 = self.eca4(x4)  # 使用ECA模块
        
        # 解码器部分
        x = self.up1(x4, x3)
        x = self.up_eca1(x)  # 使用ECA模块
        
        x = self.up2(x, x2)
        x = self.up_eca2(x)  # 使用ECA模块
        
        x = self.up3(x, x1)
        x = self.up_eca3(x)  # 使用ECA模块
        
        x = self.up4(x, x0)
        x = self.up_eca4(x)  # 使用ECA模块
